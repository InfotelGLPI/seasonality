	...
	if ($_ ne '..' && $_ ne '.'){
            ...
            if(!(-l "$dir/$_")){
                     if (index($_,".po",0)==length($_)-3) {
                        ...
                        `wget --user=$user --password=$password --output-document=locales/$_ http://www.transifex.com/api/2/project/GLPI_seasonality/resource/glpi/translation/$lang/?file=$_`;
                     }
            }
